...
# def _mix_rbf_kernel2(X, Y, sigma_list):
#     # assert(tf.shape(X) == tf.shape(Y))
#     m = tf.shape(X)

#     Z = tf.compat.v1.concat((X, Y), 0)
#     ZZT = tf.compat.v1.matmul(Z, tf.transpose(Z))
#     # diag_ZZT = tf.compat.v1.diag_part(ZZT).unsqueeze(1)
#     # Z_norm_sqr = diag_ZZT.expand_as(ZZT)
#     diag_ZZT = tf.expand_dims(tf.compat.v1.diag_part(ZZT))
#     Z_norm_sqr = diag_ZZT.view(ZZT)
#     exponent = Z_norm_sqr - 2 * ZZT + tf.transpose(Z_norm_sqr)
...
def _mix_rbf_kernel(X, Y, sigmas, wts=None):
    ...
    return K_XX, K_XY, K_YY, tf.compat.v1.reduce_sum(wts)
